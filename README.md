...
| 🔄 **资产转移记录** | 资产位置/使用人变更历史，支持跨部门转移，完整记录变更前后信息 |
| 📉 **资产折旧报表** | 支持直线法/双倍余额递减法，实时计算当前净值，累计折旧，剩余年限 |
| 🔔 **维保到期提醒** | 仪表盘展示7天/30天内到期维保，支持弹窗提醒，确保资产处于可用状态 |
...
### v2.0.1 (2026-03-29)
- 🔒 **安全修复** — 采购审批/拒绝/标记已采购接口统一使用 require_admin 装饰器
- 🔒 **安全修复** — 维保记录创建/更新接口添加 require_admin 管理员权限校验
- 🔒 **安全修复** — 待我审批列表增加当前用户过滤（避免返回其他管理员的待审批项）
- 🔒 **安全修复** — 审批流程模板列表/详情接口添加 require_admin 管理员权限
- 📝 README 安全说明章节更新
...
- ✅ **资产转移记录** — 跨部门转移，变更历史追踪
- ✅ **资产折旧报表** — 直线法/双倍余额递减法，实时净值计算
- ✅ **维保到期提醒** — 7天/30天到期提醒，仪表盘卡片展示
- ✅ **维保记录管理** — 维修/保养/巡检历史记录
